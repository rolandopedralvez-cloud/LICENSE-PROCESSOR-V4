...
def _current_user_info(request):
    # Bearer header first (every fetch()/htmx call from the app attaches
    # this -- see ntcFetch()/htmx:configRequest in base.html) -- falls back
    # to the ntc_token cookie for a PLAIN browser navigation (clicking a
    # sidebar <a href="..."> link, typing a URL, opening a bookmark), which
    # can never carry a custom Authorization header no matter what
    # JavaScript does, because the browser sends the navigation request
    # itself before any page script runs. Without this fallback, every
    # /app/* page that calls require_login (Batch Renew, Import, Analytics,
    # Location Check, Scan Review, Trash, Users, Settings, New License,
    # View/Edit a license) 401s the instant it's reached by clicking a
    # link, even for a genuinely signed-in user -- the token was real, it
    # just had no way to reach the server on that particular request. The
    # cookie is set alongside the existing token at /api/login,
    # /api/pin-login, and /api/setup-admin (see app/routers/auth.py) and
    # cleared at /api/logout; the header path is untouched and still wins
    # when both are present, so nothing about the existing fetch-based
    # /api/* flow (classic UI included) changes.
    token = request.headers.get("authorization", "").replace("Bearer ", "").strip()
    if not token:
        token = request.cookies.get("ntc_token", "").strip()
