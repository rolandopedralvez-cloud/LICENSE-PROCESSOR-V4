...
# Mirrors the token into a cookie alongside the JSON response body, so a
# PLAIN browser navigation (not a fetch()/htmx call) can still identify the
# signed-in user -- see the long comment on app.core._current_user_info for
# why this exists. httponly=True: nothing in our own JS ever needs to read
# this cookie (the token used by fetch()/htmx already comes from
# localStorage, set from this same response's JSON body), so keeping it
# server-only is strictly safer. samesite="lax" is enough since this is a
# same-origin, same-machine app (no cross-site requests need it), and
# secure=False because the app is served over plain http:// on localhost /
# the local network, not https.
def _set_token_cookie(response: Response, token: str):
    response.set_cookie(
        "ntc_token", token,
        max_age=TOKEN_ABSOLUTE_TIMEOUT_SECONDS,
        httponly=True, samesite="lax", secure=False,
    )

@router.post("/api/login")
def login(data: dict = Body(...), response: Response = None):
    import time
...
def pin_login(data: dict = Body(...), response: Response = None):
    import time
    username = (data.get("username") or "").strip()
...
def logout(request: Request, response: Response):
    ...
    response.delete_cookie("ntc_token")
    return {"ok": True}
...
def setup_admin(data: dict = Body(...), response: Response = None):
    """
