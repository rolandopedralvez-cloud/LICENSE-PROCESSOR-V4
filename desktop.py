...
# Which UI the desktop window opens to. The pywebview window has no address
# bar, so whatever this points to is the ONLY thing a person running the
# .exe / NTC.vbs ever sees -- it does not affect the classic UI at "/",
# which is untouched and still reachable by anyone hitting this same
# server from an ordinary browser instead of this window.
#   "/app/login" -> the redesigned AdminLTE-style UI (new default)
#   "/"          -> the original classic single-page UI
...
def main_entry():
    ...
    webview.create_window(
        ...
        f"http://{LOCAL_UI}:{PORT}{START_PATH}",
