...
def _start_flag_sweep():
    import threading
    # run once shortly after startup (so a fresh server catches anything
    # that happened while it was off), then on the regular interval after
    try:
        _sweep_duplicate_licenses()
    except Exception:
        pass
...
@app.post("/api/import-flags/{flag_id}/resolve")
...
@app.post("/api/import-flags/{flag_id}/ignore")
...
@app.delete("/api/import-flags/{flag_id}")
...
@app.post("/api/import-flags/rescan")
...
# ---------------------------------------------------------------- SCAN UPLOAD (quarantine)
# Scanned/photographed RSL forms get read automatically by an AI vision
# model as a starting draft, but NOTHING here reaches the real `licenses`
# table until a person opens the draft, corrects it, and explicitly
# approves it. That review step is the whole point of "quarantine" — a
# misread scan should never silently become a real record.
...
@app.get("/api/scan/field-catalog")
...
@app.post("/api/scan/{scan_id}/approve")
...
@app.post("/api/scan/{scan_id}/reject")
